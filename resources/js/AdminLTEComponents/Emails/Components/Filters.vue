          ...
          <div class="col-sm-4 my-1">
            ...
            <div class="input-group">
              ...
              <select v-model="status" class="form-control" @change="search()">
                <option value="0">Any status emails</option>
                <option value="sent">Sent emails only</option>
                <option value="scheduled">Scheduled emails only</option>
                <option value="cancelled">Cancelled emails only</option>
              </select>
            </div>
          </div>
          <div class="col-sm-4 my-1">
  ...
  methods: {
    search: function () {
      axios({
        ...
        .then((response) => {
          this.$page.props.recentEmails = response.data.data;
        })
        .catch((error) => {
          console.log(error);
        });
